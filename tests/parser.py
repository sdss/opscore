...
class ParserTests(unittest.TestCase):
    ...
    def test00(self):
        ...
        reply = rParser.parse(
            "tui.operator.actor1.actor2.actor3 911 BossICC : key=value"
        )
        hdr = reply.header
        self.assertEqual(hdr.program, "tui")
        self.assertEqual(hdr.user, "operator")
    ...
    def test02(self):
        ...
        rParser = parser.ReplyParser()
        self.assertRaises(
            parser.ParseError, rParser.parse, "tui 911 BossICC : key=value"
        )
        self.assertRaises(
            parser.ParseError, rParser.parse, "tui. 911 BossICC : key=value"
        )
        self.assertRaises(
            parser.ParseError, rParser.parse, "tui.operator. 911 BossICC : key=value"
        )
        self.assertRaises(parser.ParseError, rParser.parse, ". 911 BossICC : key=value")
        self.assertRaises(
            parser.ParseError, rParser.parse, ".. 911 BossICC : key=value"
        )
    ...
    def test05(self):
        ...
        self.roundTrip(
            rParser,
            "tui.op 911 CoffeeMakerICC : type=decaf;blend = 20:80, Kenyan,Bolivian ; now",
        )
    ...
    def testActorReplyInvalid(self):
        ...
        rParser = parser.ActorReplyParser()
        self.assertRaises(
            parser.ParseError, rParser.parse, "911 5 + key=value"
        )  # invalid code
        self.assertRaises(
            parser.ParseError, rParser.parse, "911 : key=value"
        )  # missing userId
        self.assertRaises(
            parser.ParseError, rParser.parse, ": key=value"
        )  # missing commandId and userId
        self.assertRaises(
            parser.ParseError, rParser.parse, "911 5 Fkey=value"
        )  # missing space between code and key
