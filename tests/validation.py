...
class ValidationTest(unittest.TestCase):
    def setUp(self):
        ...
        self.c12v = protoMess.Command(
            "cmd", keywords=[self.k1, self.k2], values=["1.23", 0xBEEF]
        )
        ...
        self.cmd3 = protoValid.Cmd(
            "cmd", protoTypes.Float(), protoTypes.UInt(), "(@key1 [<key2>]) [<key3>]"
        )
    ...
    def test02(self):
        ...
        self.assertEqual(
            self.c12v, self.cmd3.create("key1", ("key2", -1.2), values=[1.23, "0xbeef"])
        )
    ...
    def test03(self):
        ...
        self.assertRaises(
            KeysError,
            lambda: self.cmd1.create(
                "key1", ("key2", "abc"), ("key3", ["0xdead", "0xbeef"])
            ),
        )
        self.assertRaises(
            KeysError,
            lambda: self.cmd1.create(
                "key1", ("key2", "1.2"), ("key3", ["1.2", "0xbeef"])
            ),
        )
        self.assertRaises(
            KeysError,
            lambda: self.cmd1.create(
                "key1", ("key2", "1.2", "2.3"), ("key3", ["0xdead", "0xbeef"])
            ),
        )
    ...
    def test04(self):
        ...
        self.assertRaises(
            ValidationError,
            lambda: self.cmd1.create(("key2", "1.2"), ("key3", ["0xdead", "0xbeef"])),
        )
        self.assertRaises(
            ValidationError,
            lambda: self.cmd1.create(
                "key4", ("key2", "1.2"), ("key3", ["0xdead", "0xbeef"])
            ),
        )
        self.assertRaises(
            ValidationError,
        ...
        self.assertRaises(
            ValidationError, lambda: self.cmd3.create("key1", values=["abc", "0xbeef"])
        )
